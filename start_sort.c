...
int	evaluate(t_list *stb, int min1, int min2)/*FIRST OFF i IS ALREADY CALLED AND INITIATED, SO THIS SEEMS LIKE A PROBLEM*/
{
	...
	while ((ft_lstsize(stb) != min1) && ft_atoi((stb)->ct) != min1)
		i++;	/*WHY ADD TO i IF IT IS ALREADY INITIATED???????*/
	rmin1 = i;
	i = 0;
